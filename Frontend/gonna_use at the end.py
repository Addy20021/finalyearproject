...
col1, col2, col3 = st.columns([6, 0.5, 1])  # Equal width columns

# Navbar (horizontal, with buttons aligned)
with col1:
    if st.button("Home"):
        st.session_state.page = "Home"
...
# if st.button("Home"):
#     st.session_state.page = "Home"
# elif st.button("Thyroid Detection"):
#     st.session_state.page = "Thyroid Detection"
...
def home_page():
    st.markdown(
        f"""
        <div class="banner">
            <h1>Leading the way to better Health!</h1>
            <p>Discover an advanced way to assess thyroid health with image recognition and text-based analysis.</p>
        </div>
        """,
...
def contact_page():
    ...
    st.markdown(""" 
    <hr>
    <p style="text-align: center;">Thyroid Detection Facility © 2024</p>
    """, unsafe_allow_html=True)
